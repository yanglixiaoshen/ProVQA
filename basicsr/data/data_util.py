...
def paired_paths_from_lmdb(folders, keys):
    ...
    with open(osp.join(input_folder, 'meta_info.txt')) as fin:
        input_lmdb_keys = [ osp.splitext(line)[0] for line in fin]
    #print(input_lmdb_keys)

    paths = []
    for lmdb_key in input_lmdb_keys: # 原来加sort 顺序会和meta_info 打乱。
        paths.append(
            dict([(f'{input_key}_path', lmdb_key)])) # [{'lq_path': /G1Aerialxxx/001}, {'lq_path': /G1Aerialxxx/002}, ...]
    return paths
def paired_paths_from_lmdb1(folders, keys):
    """Generate paired paths from lmdb files.
